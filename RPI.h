...
public:
  typedef enum Commmands{ SetMotorPos, PosReached, Get_Sensor_Value, Sensor_Value,   // commands spine
...
#else
//////////// Baustelle, hier UART bedienen
void ProcessSpine(CMouseCtrlBase::typCmd cmd, int val1=0, int val2=0, int val3=0);
//void LogMessage(CString) {}
#endif
...
private:
  ...
  CKoord ptLeg;    // x/y destination pos in move, current pos !in move
        // ptNext;   // Next Point on walking line
...
public:
  void ReceiveMsg(typCmd cmd, int val1=0, int val2=0, int val3=0);
