...
void Kinematics::createWaypoints(int Ax, int Ay, int Bx, int By, int duration)
{
    //Auflösung in 2,5mm schritte
    double resolution = 2.5;
    //Distance between the points
    double dist_points = sqrt( pow(Ax - Bx,2) + pow(Ay - By,2) );
    //Amount of steps between the two points:
    double num_steps = round(dist_points / resolution);
    //Speed calculation
    double speed = dist_points / duration;
    //----create waypoints------
    double _waypoints[2][2000] = {{0}}; //!!! How is this done in a roper way?
    //use line equation: point = StartPoint + (EndPoint-StartPoint) * step
    //we traverse along the direction vector of the line
    for (int i = 0; i < num_steps; ++i) {
        _waypoints[1][i] = Ax + ((Bx - Ax) * ((1/num_steps) * i));
        _waypoints[2][i] = Ay + ((By - Ay) * ((1/num_steps) * i));;
    }
    //from here call IK-Algorithms for the waypoints
}
*/
